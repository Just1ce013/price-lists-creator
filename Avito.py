...
class Avito(Ploschadka):
    ...
    def get_xlsx(self):
        ...
        popular_codes = []
        popular_names = []
        df = pd.read_csv("C:/Users/Сергей Обласов/Downloads/Популярные товары.csv", converters = {'Код' : str, 'Артикул': str, 'Номенклатура': str}, delimiter = ";", names = ['Код', 'Артикул', 'Номенклатура'])
        for index, row in df.iterrows():
            if len(row['Код']) < 6:
                row['Код'] = "0" + row['Код'] 
            popular_articls.append(row['Артикул'])
            popular_codes.append(row['Код'])
            popular_names.append(row['Номенклатура'])
        while True:
            ...
            for detail in details:
                ...
                for rv in replace_values:
                    a = a.replace(rv, ' ')
                if (detail["article"] in popular_articls or detail["code"] in popular_codes or detail["title"].replace(" " + detail["article"], "") in popular_names) and links != "":
                    self.data.append([
                        detail["external_id"],
                        'Package',
                        'x5_7',
