...
def yandex_new(session):
        ...
        for detail in details:
            ...
            if len(pic_links) > 0:
                for pic in pic_links:
                    ...
                    if i <= 9:
                        try:
                            pic_size = os.stat("D:/parsing/pic/" + detail["external_id"] + "_" + str(i) + ".jpeg").st_size/(1024*1024)
                        except:
                            wget.download(pic, "D:/parsing/pic/" + detail["external_id"] + "_" + str(i) + ".jpeg")
                            pic_size = os.stat("D:/parsing/pic/" + detail["external_id"] + "_" + str(i) + ".jpeg").st_size/(1024*1024)
                        if pic_size < 5:
                            picture = et.SubElement(offer, 'picture')
                            picture.text = pic
    ...
    my_file = open("yandex_2.xml", "w", encoding="utf-8")
    my_file.write(minidom.parseString(et.tostring(yml_catalog)).toprettyxml(indent = "   "))
